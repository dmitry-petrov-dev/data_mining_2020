...
class Parse5kaByCatalog(Parse5ka):
    def __init__(self, start_url, url_catalogs):
        self.url_catalogs = url_catalogs
        super().__init__(start_url)

    def get_categories(self, url):
        response: requests.Response = self._get(url, params='', headers=self._headers)
        return response.json()

    def run(self):
        for category in self.get_categories(self.url_catalogs):
            data = {'parent_group_code': category['parent_group_code'],
            ...
            self._save_to_file(data, category['parent_group_code'])


if __name__ == '__main__':
    parser = Parse5kaByCatalog('https://5ka.ru/api/v2/special_offers/', 'https://5ka.ru/api/v2/categories/')
    parser.run()
