...
from lesson3_SQlite.database import DataBase
    ...
    def page_parse(self, soup, url) -> dict:
        return {
            'post': {
                ...
                'picture': soup.find('div', attrs={'class': 'blogpost-content'}).find('img').get('src') if soup.find(
            ...
            'tags': self.collect_tags(soup)
        }

    def collect_tags(self, soup):
        tags_list = []
